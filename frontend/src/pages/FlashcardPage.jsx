...
function StatsView({ token, deckId, deckName, onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setStats(await getDeckStats(token, deckId));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  if (!deckId)
    return <p className="text-sm text-gray-400 dark:text-slate-500">Select a deck first.</p>;
  if (loading)
    return (
  ...
  if (error)
    return (
      <div className="text-sm text-red-600 bg-red-50 dark:bg-red-500/10 px-4 py-2 rounded-xl">
        {error}
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button
          onClick={onBack}
          className="w-10 h-10 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 flex items-center justify-center text-gray-500 transition-colors text-lg"
        >
          ←
        </button>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          {
            label: 'Learned',
            value: `${stats.learned} (${pct(stats.learned, stats.total_cards)}%)`,
            color: 'text-blue-600',
          ...
          <div
            key={label}
            className={`${bg} rounded-2xl px-5 py-5 text-center border border-gray-100 dark:border-slate-700`}
          >
...
export default function FlashcardPage() {
  ...
  const loadCards = useCallback(
    async (deckId) => {
      ...
      try {
        setCards(await listCards(token, deckId));
      } catch {
        /* ignore */
      } finally {
  ...
  async function handleCreateDeck(data, onDone) {
    ...
    try {
      ...
      toast.error(e.message || 'Failed to create deck.');
    } finally {
      setCreating(false);
    }
  }
  ...
  async function handleEditDeck(data, onDone, deckId) {
    ...
    try {
      ...
      toast.error(e.message || 'Failed to update deck.');
    } finally {
      setCreating(false);
    }
  }
  ...
  async function handleSubmitReview(cardId, rating) {
    try {
      await submitReview(token, cardId, rating);
    } catch {
      /* ignore */
    }
  }
  ...
  if (view === 'study') {
    return (
      <StudyMode cards={dueCards} onSubmitReview={handleSubmitReview} onClose={handleStudyClose} />
    );
  }
          ...
          <div className="flex items-center gap-2">
            {view === 'cards' && selectedDeckId && (
              <button
                ...
                className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 font-medium transition-colors"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 font-medium transition-colors"
            >
