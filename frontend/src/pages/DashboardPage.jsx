...
function SessionCard({ session, onView, onDelete, highlight = false }) {
  ...
  return (
    ...
    >
      {highlight && (
        <span className="absolute -top-2 left-4 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-blue-600 text-white shadow-sm animate-fadeIn">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-gray-400 mt-0.5">{fmtDate(session.date, lang)}</div>
          </div>
        </div>
        <span
          style={{
          ...
          className={`text-sm font-bold px-3 py-1 rounded-full ${
            session.isReal && onDelete ? 'group-hover:opacity-0 transition-opacity' : ''
          }`}
        >
          {session.avgScore}
        </span>
...
export default function DashboardPage({ demoMode = false }) {
  ...
  const [profile, setProfile] = useState(() =>
    demoMode ? { display_name: 'Demo User', email: 'demo@example.com' } : null
  );
  ...
  const topicFilterOptions = useMemo(() => {
    const unique = [...new Set(realSessions.map(s => s.topic).filter(Boolean))];
  ...
  const filtered = useMemo(() => {
    const byTab =
      activeTab === 'All' ? allSessions : allSessions.filter(s => s.topic === activeTab);
  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-[#f5f7fa] flex items-center justify-center">
        <div className="text-gray-400 text-sm animate-pulse">{t('dash.loading')}</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f5f7fa] text-gray-900">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-base font-semibold text-gray-800">{t('brand.name')}</span>
        </div>
        <div className="flex items-center gap-3">
          <LanguageToggle />
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <section className="mb-10">
          <div className="flex items-end justify-between mb-5">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">{t('dash.topics.title')}</h2>
              <p className="text-sm text-gray-500 mt-1">{t('dash.topics.subtitle')}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-5 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-lg font-bold text-gray-900">{t('dash.history.title')}</h2>
