      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700 max-w-md w-full p-6"
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-5">
              ...
              <span className="font-semibold text-gray-700 dark:text-white">{deckName}</span>? This
              will delete the deck and all its cards.
            </p>
    ...
    <div>
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-52 rounded-2xl bg-gray-100 dark:bg-slate-800 animate-pulse" />
          ))}
        </div>
      ) : filtered.length > 0 ? (
        <motion.div
          ...
          {filtered.map((deck) => {
            ...
            return (
              <motion.div key={deck.id} variants={item}>
                <div
                  className="group relative rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden cursor-pointer hover:shadow-lg hover:border-blue-300 dark:hover:border-blue-600 transition-all duration-200"
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-3 space-y-1">
                      <h3 className="font-semibold text-base leading-tight line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors dark:text-white">
                        {deck.name}
                      </h3>
                    ...
                    <div className="mt-4 flex items-center gap-4 text-sm">
                      <span className="flex items-center gap-1.5 text-gray-500 dark:text-slate-400">
                        <Layers className="h-3.5 w-3.5" /> {deck.card_count ?? 0} cards
                      </span>
                      {hasDue && (
                        <span className="flex items-center gap-1.5 text-orange-500 font-medium">
                          <Play className="h-3.5 w-3.5 fill-orange-500" /> {deck.due_count} due
                          today
                        </span>
                      )}
                    </div>
