...
import { Plus, Search, MoreVertical, Edit, Trash2, Play, Layers, ArrowLeft, Image, Volume2 } from 'lucide-react';
...
function CardFormDialog({ mode = 'create', card, onSubmit, submitting, trigger }) {
  ...
  function handleOpen() {
    if (mode === 'edit' && card) { setFront(card.front_text || ''); setBack(card.back_text || ''); setTags(card.tags?.join(', ') || ''); }
    setOpen(true);
  }
  ...
  function handleSubmit(e) {
    ...
    onSubmit(
      ...
      () => { if (mode === 'create') { setFront(''); setBack(''); setTags(''); setMediaFiles([]); } setOpen(false); },
  ...
  return (
    <>
      {trigger ? (
        ...
        <button onClick={handleOpen} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-all shadow-sm hover:shadow-md active:scale-95">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setOpen(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
            className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700 max-w-2xl w-full p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold dark:text-white mb-1">{mode === 'create' ? 'Add New Card' : 'Edit Card'}</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-5">Provide the front and back content for this flashcard.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Front (Question / Prompt)</label>
                  <textarea value={front} onChange={e => setFront(e.target.value)} placeholder="What is..." rows={5} autoFocus
                    className="w-full rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm dark:text-white resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow min-h-[150px]" required />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Back (Answer)</label>
                  <textarea value={back} onChange={e => setBack(e.target.value)} placeholder="The answer is..." rows={5}
                    className="w-full rounded-lg border border-gray-200 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50 px-3 py-2 text-sm dark:text-white resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow min-h-[150px]" required />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-gray-700 dark:text-slate-300">Tags (comma separated)</label>
                <input value={tags} onChange={e => setTags(e.target.value)} placeholder="e.g. react, hooks, frontend"
              ...
              {mode === 'create' && (
                <div className="space-y-2">
                  {mediaFiles.map((item, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2 bg-gray-50 dark:bg-slate-800 rounded-lg px-3 py-2 text-sm">
                        <Icon className="h-4 w-4 text-gray-500" />
                        <span className="flex-1 truncate text-xs text-gray-600 dark:text-slate-300">{item.file.name}</span>
                        <select value={item.side} onChange={e => setMediaFiles(prev => prev.map((m, j) => j === i ? { ...m, side: e.target.value } : m))}
                        ...
                        <button type="button" onClick={() => setMediaFiles(prev => prev.filter((_, j) => j !== i))} className="text-red-400 hover:text-red-600 transition-colors">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-sm text-blue-600 hover:text-blue-700 w-fit transition-colors">
                    ...
                    <input type="file" accept="image/*,audio/*" multiple className="hidden" onChange={handleFiles} />
                  </label>
                </div>
              )}
              <div className="flex gap-2 justify-end pt-2">
                ...
                <button type="submit" disabled={submitting} className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold disabled:opacity-50 transition-all active:scale-95">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setOpen(false)}>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }}
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-5">Are you sure? This action cannot be undone.</p>
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => { onDelete(cardId); setOpen(false); }} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-all active:scale-95">Delete Card</button>
            </div>
          </motion.div>
        </div>
      )}
      ...
      {open && (
        <>
          ...
          <motion.div initial={{ opacity: 0, y: -4, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.15 }}
            ...
            <CardFormDialog mode="edit" card={card} onSubmit={onEdit} submitting={submitting}
              trigger={
            ...
            <DeleteCardDialog cardId={card.id} onDelete={(id) => { onDelete(id); setOpen(false); }} />
          </motion.div>
        </>
      )}
    </div>
...
export default function CardList({ cards, loading, deckName, onBack, onCreateCard, onEditCard, onDeleteCard, onStudy, creating }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={onBack} className="w-10 h-10 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 flex items-center justify-center text-gray-500 dark:text-slate-400 transition-colors">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <h2 className="flex-1 text-2xl font-bold dark:text-white">{deckName || 'Cards'}</h2>
        <button onClick={onStudy} disabled={!cards?.length}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl p-4">
        <div className="relative flex-1 w-full">
          ...
          <input type="search" placeholder="Search cards or tags..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {/* Cards */}
      {loading ? (
        <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-32 w-full rounded-xl bg-gray-100 dark:bg-slate-800 animate-pulse" />)}</div>
      ) : filtered.length > 0 ? (
            ...
            <motion.div key={card.id} variants={item}
              ...
              <div className="flex flex-col md:flex-row">
                <div className="flex-1 p-4 border-b md:border-b-0 md:border-r border-gray-100 dark:border-slate-800">
                  <div className="text-xs font-medium text-gray-400 dark:text-slate-500 mb-2 tracking-wider">FRONT</div>
                  <div className="text-sm dark:text-white whitespace-pre-wrap">{card.front_text}</div>
                  {card.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-3">
                      {card.tags.map(tag => (
                        <span key={tag} className="text-[10px] font-medium bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 px-2 py-0.5 rounded-full">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
                <div className="flex-1 p-4 bg-gray-50/50 dark:bg-slate-800/30 relative">
                  <div className="absolute top-2 right-2">
                    <CardDropdown card={card} onEdit={onEditCard} onDelete={onDeleteCard} submitting={creating} />
                  </div>
                  <div className="text-xs font-medium text-gray-400 dark:text-slate-500 mb-2 tracking-wider">BACK</div>
                  <div className="text-sm dark:text-slate-200 pr-8 whitespace-pre-wrap">{card.back_text}</div>
                  {card.media?.length > 0 && (
                      ...
                      {card.media.map(m => {
                        ...
                        return (
                          <span key={m.id} className="text-xs text-gray-400 flex items-center gap-1">
                            <Icon className="h-3.5 w-3.5" /> <span className="capitalize">{m.side}</span>
                          </span>
                        );
                      })}
                    </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed rounded-xl bg-gray-50/50 dark:bg-slate-900/50">
          ...
          <p className="text-gray-500 dark:text-slate-400 max-w-sm mb-4 text-sm">
            {search ? 'No cards match your search.' : 'This deck is empty. Add some cards to start studying.'}
          </p>
          {!search && <CardFormDialog mode="create" onSubmit={onCreateCard} submitting={creating} />}
        </div>
      )}
    </div>
  );
