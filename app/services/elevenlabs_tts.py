...
_LEGACY_GENDER_VOICE_ENV_MAP: dict[str, str] = {
...
class ElevenLabsTTS:
    ...
    def _resolve_voice_id(self, voice_gender: str | None = None, voice_accent: str | None = None) -> str:
        ...
        if env_key:
            ...
            legacy_env_key = _LEGACY_GENDER_VOICE_ENV_MAP.get(gender)
            if legacy_env_key:
                legacy_voice_id = self._get_env_value(legacy_env_key)
                if legacy_voice_id:
                    logger.warning(
                        "ElevenLabsTTS: using legacy %s fallback for gender=%r accent=%r",
                        legacy_env_key,
                        gender,
                        accent,
                    )
